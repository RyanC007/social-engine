...
def publish_instagram(post: PlatformPost, media_urls: list[str] = None,
 ...
 Instagram enforces a maximum of 5 hashtags per post -- enforced automatically.
 """
 import copy
 acct = config.BLOTATO_ACCOUNTS["instagram"]
 # Instagram hard limit: 5 hashtags max
 ig_post = post
 if hasattr(post, 'hashtags') and len(post.hashtags) > 5:
 ig_post = copy.copy(post)
 ig_post.hashtags = post.hashtags[:5]
 target_extra = {
