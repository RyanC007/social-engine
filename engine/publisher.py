...
def _truncate_instagram_hashtags(text: str, max_hashtags: int = 5) -> str:
 """
 Instagram enforces a maximum of 5 hashtags per post.
 Splits the text on the hashtag block (last paragraph), keeps only the
 first max_hashtags tags, and reassembles the caption.
 """
 lines = text.split("\n")
 # Find the hashtag line (last non-empty line that starts with #)
 hashtag_line_idx = None
 for i in range(len(lines) - 1, -1, -1):
 if lines[i].strip() and all(w.startswith('#') for w in lines[i].split()):
 hashtag_line_idx = i
 break
 if hashtag_line_idx is not None:
 tags = lines[hashtag_line_idx].split()
 lines[hashtag_line_idx] = ' '.join(tags[:max_hashtags])
 else:
 # Hashtags may be inline -- strip extras from the whole text
 all_tags = [w for w in text.split() if w.startswith('#')]
 if len(all_tags) > max_hashtags:
 keep = set(all_tags[:max_hashtags])
 seen = set()
 new_words = []
 for w in text.split():
 if w.startswith('#'):
...
def publish_instagram(cfg: ClientConfig, text: str, media_urls: list = None,
 ...
 # Instagram hard limit: max 5 hashtags per post caption
 ig_text = _truncate_instagram_hashtags(text, max_hashtags=5)
 target_extra = {
