...
from engine.publisher import (
    ...
    create_youtube_short_video, create_linkedin_image,
...
def _generate_linkedin_image(cfg: ClientConfig, pillar) -> str | None:
    ...
    NOTE: The returned URL is also reused for Facebook to save generation credits.
    """
...
def run_publish(
    ...
    Credit optimizations applied:
    - LinkedIn image is generated once and reused for Facebook (saves 15 credits/post)
    - YouTube Short video is generated once and reused for TikTok (saves ~50 credits/post)
    """
    cfg = load_client(client_slug)
    folder = (
        get_week_folder_by_name(cfg, week) if week
    ...
    # CREDIT OPTIMIZATION: The same image URL is captured and reused for
    # Facebook below, eliminating a duplicate 15-credit generation call.
    # -----------------------------------------------------------------------
    shared_linkedin_image_url = None
    if "linkedin" in target_platforms and linkedin_post.text:
        shared_linkedin_image_url = _generate_linkedin_image(cfg, pillar)
        media_urls = [shared_linkedin_image_url] if shared_linkedin_image_url else (brand_images[:1] if brand_images else [])
        try:
    ...
    # -----------------------------------------------------------------------
    # Facebook - reuse the LinkedIn image instead of generating a new one.
    # Falls back to brand_images[0] if LinkedIn was not in target_platforms.
    # -----------------------------------------------------------------------
    def _publish_facebook_reuse_image(p):
        """Reuse the LinkedIn image for Facebook - no extra generation needed."""
        fb_image = shared_linkedin_image_url or (brand_images[0] if brand_images else None)
        print(f"    Facebook: reusing LinkedIn image (no extra generation credit used)")
        return publish_facebook(
            cfg,
            text=p.formatted(),
            page_key=cfg.default_facebook_page(),
            media_urls=[fb_image] if fb_image else None,
            schedule_at=schedule_at,
        )
    ...
    platform_map = {
        ...
        "facebook":  _publish_facebook_reuse_image,
    }

    # -----------------------------------------------------------------------
    # Video - generate once, shared by YouTube Shorts and TikTok.
    # -----------------------------------------------------------------------
